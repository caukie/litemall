  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-form-item label="所属平台">
        <el-select v-model="listQuery.supplierPlatformId" clearable>
          <el-option v-for="item in supplierPlatformList" :key="item.id" :label="item.label" :value="item.name" />
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="供应商ID" prop="id" />

      <el-table-column align="center" label="供应商名称" prop="name" />

      <el-table-column align="center" label="所属平台" prop="platformName" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="供应商商名称" prop="name">
          <el-input v-model="dataForm.name" />
        </el-form-item>
        <el-form-item label="所属平台">
          <el-select v-model="dataForm.supplierPlatformId" clearable>
            <el-option v-for="item in supplierPlatformList" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
...
<script>
import { listSupplier, createSupplier, updateSupplier, deleteSupplier, listSupplierPlatform } from '@/api/supplier'
...
export default {
  ...
  methods: {
    getPlatform() {
      listSupplierPlatform()
        .then(response => {
          this.supplierPlatformList = response.data.data.list
        })
        .catch(() => {
          this.supplierPlatformList = []
        })
    },
    getList() {
      this.listLoading = true
      listSupplier(this.listQuery)
        .then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const filterVal = ['id', 'name', 'desc', 'floorPrice', 'supplierPlatformId', 'platformName']
